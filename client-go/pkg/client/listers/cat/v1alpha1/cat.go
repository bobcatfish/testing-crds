/*
Copyright The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by lister-gen. DO NOT EDIT.

package v1alpha1

import (
	v1alpha1 "github.com/bobcatfish/testing-crds/pkg/apis/cat/v1alpha1"
	"k8s.io/apimachinery/pkg/api/errors"
	"k8s.io/apimachinery/pkg/labels"
	"k8s.io/client-go/tools/cache"
)

// CatLister helps list Cats.
type CatLister interface {
	// List lists all Cats in the indexer.
	List(selector labels.Selector) (ret []*v1alpha1.Cat, err error)
	// Cats returns an object that can list and get Cats.
	Cats(namespace string) CatNamespaceLister
	CatListerExpansion
}

// catLister implements the CatLister interface.
type catLister struct {
	indexer cache.Indexer
}

// NewCatLister returns a new CatLister.
func NewCatLister(indexer cache.Indexer) CatLister {
	return &catLister{indexer: indexer}
}

// List lists all Cats in the indexer.
func (s *catLister) List(selector labels.Selector) (ret []*v1alpha1.Cat, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Cat))
	})
	return ret, err
}

// Cats returns an object that can list and get Cats.
func (s *catLister) Cats(namespace string) CatNamespaceLister {
	return catNamespaceLister{indexer: s.indexer, namespace: namespace}
}

// CatNamespaceLister helps list and get Cats.
type CatNamespaceLister interface {
	// List lists all Cats in the indexer for a given namespace.
	List(selector labels.Selector) (ret []*v1alpha1.Cat, err error)
	// Get retrieves the Cat from the indexer for a given namespace and name.
	Get(name string) (*v1alpha1.Cat, error)
	CatNamespaceListerExpansion
}

// catNamespaceLister implements the CatNamespaceLister
// interface.
type catNamespaceLister struct {
	indexer   cache.Indexer
	namespace string
}

// List lists all Cats in the indexer for a given namespace.
func (s catNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Cat, err error) {
	err = cache.ListAllByNamespace(s.indexer, s.namespace, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Cat))
	})
	return ret, err
}

// Get retrieves the Cat from the indexer for a given namespace and name.
func (s catNamespaceLister) Get(name string) (*v1alpha1.Cat, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("cat"), name)
	}
	return obj.(*v1alpha1.Cat), nil
}
